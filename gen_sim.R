...
mkResp <- function(sDat, betas, form, debug=FALSE){
...
mkSim <- function(resp_list, mod, s_size, cMat, mList, betas,fprob, sprob, prList, debug=FALSE){
    fates <- names(fprob)
    spp   <- names(sprob)
    ...
    cat("\nmerged sim data for the two species")
